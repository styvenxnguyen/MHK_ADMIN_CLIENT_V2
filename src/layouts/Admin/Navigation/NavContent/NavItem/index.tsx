...
const NavItem = ({ layout, item }: any) => {
  ...
  if (layout === 'horizontal') {
    mainContent = (
      <ListGroup.Item as='li' bsPrefix=' ' onClick={() => dispatch({ type: actionType.NAV_CONTENT_LEAVE, value: '' })}>
        {subContent}
      </ListGroup.Item>
    )
  } else {
    if (windowSize.width < 992) {
      mainContent = (
        <ListGroup.Item
          ...
          onClick={() => dispatch({ type: actionType.COLLAPSE_MENU, value: '' })}
        >
          {subContent}
        </ListGroup.Item>
      )
